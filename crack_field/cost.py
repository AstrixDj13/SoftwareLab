...
def getElasticEnergy(u, x,y, s, weight, jacobian, E, nu, Gc, eps):
    ...
    K = G/(1 - 2*nu(x,y))
    eta = 1
    phi_dev = 0.5*K*(((eux + evy - torch.abs(eux + evy))*0.5)**2)
    phi_vol = 0.5*K*(((eux + evy + torch.abs(eux + evy))*0.5)**2) + G*(eux**2 + evy**2 + 0.5*(euy + evx)**2)
    I1 = (s**2 + eta)*phi_vol + phi_dev
...
def costFunction(U, x,y, weight, jacobian, p, E, nu,s,Gc,eps):
    return getExternalEnergy(U, x,y, weight, jacobian,p) + getElasticEnergy(U, x,y, s,weight, jacobian, E, nu, Gc, eps)
