...
def trainModel(model, X, x, y, s0, u0, weights, jacobian, domainLengthX, domainLengthY, p, E, nu, eps, Gc, optimizer, epochs, val_X, val_x, val_y, val_s, val_weights, val_jacobian, analyticalSolution,ts):
    ...
    for time in range(ts):
        for i in range (epochs):
            ...
            loss, pureElasticEnergyDensity, crackEnergyDensity = cost.costFunction(U, x,y, weights,jacobian, p,E,nu,s,Gc,eps,ds)
            ...
            elasticEnergyData.append(pureElasticEnergyDensity.detach())
            crackEnergyData.append(crackEnergyDensity.detach())
