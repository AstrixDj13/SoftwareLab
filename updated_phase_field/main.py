...
s0 = (y>=0.5)*(1 - torch.exp(-(torch.abs(x - 0.5 * domainLengthX) / eps))) + (y<0.5) # Half crack in y direction
#s0 = (x>=0.5)*(1 - torch.exp(-(torch.abs(y - 0.5 * domainLengthX) / eps))) + (x<0.5) # Half crack in x direction
# discontinuous
...
U, s, epochData, costData = PINN_2d.trainModel(model, X, x, y, s0, u0, weights, jacobian, domainLengthX, domainLengthY,
                                               p, E, nu, eps, Gc, optimizer, epochs)
