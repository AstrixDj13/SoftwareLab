...
numTP = 10
numGP = 4
x = torch.linspace(0, 2, numTP)
y = torch.linspace(0, 2, numTP)

[global_map, weights, jacobian] = getGlobalMapping(x,y,numGP,numGP)
integrand = torch.ones(numGP**2*(numTP-1)**2).view(-1,1)

print(gaussianIntegration(integrand,weights,jacobian))
"""